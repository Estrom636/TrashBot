    ...
    if (Serial1.read() == HEADER) {
      ...
      if (CalCRC8((uint8_t*)&lidarFrame, sizeof(LiDARFrame) - 1) == lidarFrame.crc8) {
        ...
        for (int i = 0; i < POINTS_PER_PACK; i++) {
          ...
          // Filter: Adjust distance > 10 to ignore the robot's own body
...
void debugPrintZones() {
  ...
  Serial.printf("ARMED: %s L: %.2f R: %.2f Slow: %s Controller: %s \n", armed ? "TRUE" : "FALSE", lPower, rPower, slow ? "true" : "false", controller ? "true" : "false");
}