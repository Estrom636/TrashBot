#include <ESP32Servo.h>
#include <Bluepad32.h>
#include <math.h>

// --- LIDAR DEFINITIONS ---
#define POINTS_PER_PACK 12
#define HEADER 0x54
#define LIDAR_BAUD 230400
#define LIDAR_PIN 5
#define LIDAR_PWM_PIN 18

typedef struct __attribute__((packed)) {
  uint16_t distance;
  uint8_t intensity;
} LidarPoint;

typedef struct __attribute__((packed)) {
  uint8_t header;
  uint8_t ver_len;
  uint16_t speed;
  uint16_t start_angle;
  LidarPoint point[POINTS_PER_PACK];
  uint16_t end_angle;
  uint16_t timestamp;
  uint8_t crc8;
} LiDARFrame;

// --- GLOBAL VARIABLES ---
const int encoderAL = 39;
const int encoderBL = 36;
const int encoderAR = 34;
const int encoderBR = 35;

const float pulsesPerRevolution = 385.4;
const double DIAMETERmm = 72;
const double DIAMETERin = DIAMETERmm/25.4;
const double CIRCUMFERENCE = DIAMETERin * PI;
const double TRACKWIDTHmm = 586;
const double TRACKWIDTHin = TRACKWIDTHmm/25.4;

ControllerPtr myControllers[BP32_MAX_CONTROLLERS];
Servo light;
Servo motorL;
Servo motorR;
Servo lidarMotor;

float zoneSums[16] = {0};
int zoneCounts[16] = {0};
float zoneAverages[16] = {0};
unsigned long lastAverageTime = 0;

double lPower = 0;
double rPower = 0;
bool armed = false;
bool armedOverRide = false;
bool slow = false;
bool controller = false;
bool isAuto = false;
bool isMoving = false;

float rotationsL;
float rotationsR;

volatile long encoderTicksL = 0;
volatile long encoderTicksR = 0;

LiDARFrame lidarFrame;
static const uint8_t CrcTable[256]={
    
};

// --- HELPER FUNCTIONS ---
void IRAM_ATTR handleEncoderL() {
  int sA = digitalRead(encoderAL);
  int sB = digitalRead(encoderBL);

  static int lastA = 0;
  static int lastB = 0;

  if (sA != lastA || sB != lastB) {
    if ((lastA == LOW && sA == HIGH && sB == LOW) || 
        (lastA == HIGH && sA == LOW && sB == HIGH) ||
        (lastB == LOW && sB == HIGH && sA == HIGH) ||
        (lastB == HIGH && sB == LOW && sA == LOW)) {
      encoderTicksL++;
    } else {
      encoderTicksL--;
    }
  }
  lastA = sA;
  lastB = sB;
}

void IRAM_ATTR handleEncoderR() {
  int sA = digitalRead(encoderAR);
  int sB = digitalRead(encoderBR);

  static int lastA = 0;
  static int lastB = 0;

  if (sA != lastA || sB != lastB) {
    if ((lastA == LOW && sA == HIGH && sB == LOW) || 
        (lastA == HIGH && sA == LOW && sB == HIGH) ||
        (lastB == LOW && sB == HIGH && sA == HIGH) ||
        (lastB == HIGH && sB == LOW && sA == LOW)) {
      encoderTicksR++;
    } else {
      encoderTicksR--;
    }
  }
  lastA = sA;
  lastB = sB;
}

uint8_t CalCRC8(uint8_t *p, uint8_t len) {
  uint8_t crc = 0;
  for (int i = 0; i < len; i++) {
    crc = CrcTable[(crc ^ *p++) & 0xff];
  }
  return crc;
}

void MotorPowL(double power) {
  if(power >  1) power =  1;
  if(power < -1) power = -1;

  motorL.write(90 + (power * 90));
}
void MotorPowR(double power) {
  if(power >  1) power =  1;
  if(power < -1) power = -1;

  motorR.write(90 + (power * -90));
}

// --- BLUEPAD32 CALLBACKS ---
void onConnectedController(ControllerPtr ctl) {
  controller = true;
  for (int i = 0; i < BP32_MAX_GAMEPADS; i++) {
    if (myControllers[i] == nullptr) {
      myControllers[i] = ctl;
      Serial.printf("Controller connected at index %d\n", i);
      break;
    }
  }
}

void onDisconnectedController(ControllerPtr ctl) {
  armed = false;
  controller = false;
  for (int i = 0; i < BP32_MAX_GAMEPADS; i++) {
    if (myControllers[i] == ctl) {
      myControllers[i] = nullptr;
      Serial.printf("Controller disconnected from index %d\n", i);
      break;
    }
  }
}

void processGamepad(ControllerPtr gamepad) {
  if (gamepad->throttle()){
    armedOverRide = true;
  }else{
    armedOverRide = false;
  }
  if (gamepad->miscStart()) {
    armed = true;
    gamepad->setRumble(100, 75);
  }
  if (gamepad->miscSelect()) {
    armed = false;
  }
  if (gamepad->a() && !isAuto) {
    isAuto = true;
    moveDistance(20, 20);
    delay(100);
    rotateDegrees(90);
    isAuto = false;
  }
  if (gamepad->x()) {
    isAuto = false;
  }
  // Simplified power mapping
  lPower = gamepad->axisY() / -512.0;
  rPower = gamepad->axisRY() / -512.0;
}

void updateAll() {
  BP32.update();

  noInterrupts();
  long currentTicksL = encoderTicksL;
  long currentTicksR = encoderTicksR;
  interrupts();
  rotationsL = currentTicksL / pulsesPerRevolution;
  rotationsR = currentTicksR / pulsesPerRevolution;

  // Handle Bluetooth Input
  for (int i = 0; i < BP32_MAX_CONTROLLERS; i++) {
    ControllerPtr myController = myControllers[i];
    if (myController && myController->isConnected() && myController->isGamepad()) {
      processGamepad(myController);
    }
  }

  updateLidar(); 
  updateZones(100);
  slow = checkZones();
}

// --- SETUP ---
void setup() {
  Serial.begin(115200);
  Serial1.begin(LIDAR_BAUD, SERIAL_8N1, LIDAR_PIN, -1);

  // Allocate Timers for PWM
  ESP32PWM::allocateTimer(0);
  ESP32PWM::allocateTimer(1);
  ESP32PWM::allocateTimer(2);
  ESP32PWM::allocateTimer(3);

  // Attach Servos and Motors
  light.setPeriodHertz(50);
  light.attach(33, 1000, 2500);

  motorL.setPeriodHertz(50);
  motorL.attach(26, 1000, 1950);

  motorR.setPeriodHertz(50);
  motorR.attach(27, 1000, 1950);

  // Handle Lidar Motor as a Servo to keep timers synced
  lidarMotor.setPeriodHertz(50); 
  lidarMotor.attach(LIDAR_PWM_PIN);
  lidarMotor.write(180); // Adjust this value (0-180) to set LiDAR spin speed

  BP32.setup(&onConnectedController, &onDisconnectedController);
  BP32.forgetBluetoothKeys();

  pinMode(encoderAL, INPUT_PULLUP);
  pinMode(encoderBL, INPUT_PULLUP);

  pinMode(encoderAR, INPUT_PULLUP);
  pinMode(encoderBR, INPUT_PULLUP);

  attachInterrupt(digitalPinToInterrupt(encoderAL), handleEncoderL, CHANGE);
  attachInterrupt(digitalPinToInterrupt(encoderBL), handleEncoderL, CHANGE);

  attachInterrupt(digitalPinToInterrupt(encoderAR), handleEncoderR, CHANGE);
  attachInterrupt(digitalPinToInterrupt(encoderBR), handleEncoderR, CHANGE);
}

// --- LOOP ---
void loop() {
  updateAll();

  if(armedOverRide) armed = true;

  // Handle Armed State Actions
  if (armed) {
    // Blinking Light logic
    if (((millis() / 1000) % 2) == 0) {
      light.write(30);
    } else {
      light.write(0);
    }
    if(slow){
      MotorPowL(lPower * 0.4);
      MotorPowR(rPower * 0.4);
    }else{
      MotorPowL(lPower * 0.6);
      MotorPowR(rPower * 0.6);
    }
  } else {
    light.write(0);
    MotorPowL(0);
    MotorPowR(0);
  }
}

bool checkZones(){
  checkAll();
  if(checkSides() || checkFR() || checkC()){
    return true;
  }else{
    return false;
  }
}
void checkAll(){
  for(int i = 0; i < 16; i ++){
    if (zoneAverages[i] > 0 && zoneAverages[i] < 400) {
      armed = false;
    }
  }
}
bool checkSides(){
  int zones[] = {3, 4, 11, 12}; 
  for (int i = 0; i < 4; i++) {
    if (zoneAverages[zones[i]] > 0 && zoneAverages[zones[i]] < 500) {
       return true;
    }
  }
  return false;
}
bool checkFR(){
  int zones[] = {0, 7, 8, 15}; 
  for (int i = 0; i < 4; i++) {
    if (zoneAverages[zones[i]] > 0 && zoneAverages[zones[i]] < 1000) {
       return true;
    }
  }
  return false;
}
bool checkC(){
  int zones[] = {1, 2, 5, 6, 9, 10, 13, 14}; 
  for (int i = 0; i < 8; i++) {
    if (zoneAverages[zones[i]] > 0 && zoneAverages[zones[i]] < 750) {
       return true;
    }
  }
  return false;
}

void updateLidar() {

  while (Serial1.available() >= sizeof(LiDARFrame)) {

    if (Serial1.read() == HEADER) {

      lidarFrame.header = HEADER;
      Serial1.readBytes((char*)&lidarFrame + 1, sizeof(LiDARFrame) - 1);

      if (CalCRC8((uint8_t*)&lidarFrame, sizeof(LiDARFrame) - 1) == lidarFrame.crc8) {

        double step = (lidarFrame.end_angle - lidarFrame.start_angle) / (POINTS_PER_PACK - 1);

        for (int i = 0; i < POINTS_PER_PACK; i++) {

          double angle = (lidarFrame.start_angle + step * i);
          angle = fmod(angle + 36000, 36000) / 100.0;

          int distance = lidarFrame.point[i].distance;
          uint8_t intensity = lidarFrame.point[i].intensity;

          if (intensity > 20 && distance > 10 && distance < 20000) {

            int zone = (int)(angle / 22.5);

            if (zone >= 0 && zone < 16) {
              zoneSums[zone] += distance;
              zoneCounts[zone]++;
            }
          }
        }
      }
    }
  }
}

void updateZones(int interval) {
  if (millis() - lastAverageTime > interval) {
    lastAverageTime = millis();
    
    for (int z = 0; z < 16; z++) {
      //Serial.println(zoneSums[z]);
      if (zoneCounts[z] > 0) {
        zoneAverages[z] = zoneSums[z] / zoneCounts[z];
      } else {
        zoneAverages[z] = 0; // 0 indicates no object detected/out of range
      }
      
      // Reset accumulators for the next interval
      zoneSums[z] = 0;
      zoneCounts[z] = 0;
      //Serial.println("Zone Update: " + String(z));
    }
    
    // Debug print (optional)
    debugPrintZones();
  }
}

void debugPrintZones() {
  Serial.print("Zones: ");
  for(int i = 0; i < 16; i++) {
    Serial.printf("[%d]:%.0f ", i, zoneAverages[i]);
  }
  Serial.printf("ARMED: %s L: %.2f R: %.2f Slow: %s Controller: %s Auto: %s Moving: %s \n", armed ? "TRUE" : "FALSE", lPower, rPower, slow ? "true" : "false", controller ? "true" : "false", isAuto ? "true" : "false", isMoving ? "ture" : "false");
}

double getRotationsL() {
  noInterrupts();
  long ticks = encoderTicksL;
  interrupts();
  return (double)ticks / pulsesPerRevolution;
}

double getRotationsR() {
  noInterrupts();
  long ticks = encoderTicksR;
  interrupts();
  return (double)ticks / pulsesPerRevolution;
}

void resetEncoders() {
  noInterrupts();
  encoderTicksL = 0;
  encoderTicksR = 0;
  interrupts();
}

void moveDistance(double leftDis, double rightDis){

  resetEncoders();

  double leftTarget  = leftDis  / CIRCUMFERENCE;
  double rightTarget = rightDis / CIRCUMFERENCE;

  double baseSpeed = 0.25;
  double kSync = 0.2;
  double tolerance = 0.03;

  while(true){

    updateAll();

    double leftCurrent  = getRotationsL();
    double rightCurrent = getRotationsR();

    double leftError  = leftTarget  - leftCurrent;
    double rightError = rightTarget - rightCurrent;

    if((abs(leftError) < tolerance && abs(rightError) < tolerance) || !isAuto){
      break;
    }

    int leftDir  = (leftError  > 0) ? 1 : -1;
    int rightDir = (rightError > 0) ? 1 : -1;

    double leftPow  = leftDir  * baseSpeed;
    double rightPow = rightDir * baseSpeed;

    // sync correction ONLY when moving same direction
    if(leftDir == rightDir){
      double syncError = leftCurrent - rightCurrent;

      leftPow  -= kSync * syncError;
      rightPow += kSync * syncError;
    }

    if(armed){
      isMoving = true;

      if (((millis() / 500) % 2) == 0) {
        light.write(30);
      } else {
        light.write(0);
      }

      MotorPowL(leftPow);
      MotorPowR(rightPow);

    }else{

      isMoving = false;
      MotorPowL(0);
      MotorPowR(0);

    }

    delay(10);
  }

  isMoving = false;

  MotorPowL(0);
  MotorPowR(0);
}

void rotateDegrees(double degree){
  if(abs(degree) > 360) degree = 360 * (degree / abs(degree));
  double inches = (degree / 360) * PI * TRACKWIDTHin;
  
  if(degree > 0){
    //+ Rotation -> Clockwise
    moveDistance(inches, -1 * inches);
  }else{
    //- Rotation -> Counterclockwise
    moveDistance(-1 * inches, inches);
  }
}

int direction(double num){
  return num / abs(num);
}